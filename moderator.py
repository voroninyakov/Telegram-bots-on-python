...
your_id = ''  # id в телеграмме того, кто сможет управлять ботом

with open('chats_name.txt', 'w+', encoding='utf-8'):  # создаём файл с именами чатов
    pass
with open('chats_id.txt', 'w+', encoding='utf-8'):  # создаём файл с id чатов
    pass
...
def starts(message):
    if message.from_user.username == your_id:
